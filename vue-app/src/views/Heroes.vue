  ...
  <div class="card text-center m-3" v-if="this.gotData == 'yes'">
    ...
    <div class="card-body">
      <div v-for="item in pageOfItems" :key="item.id">
        <div class="hero">
          <router-link
            :to="{ name: 'Hero', params: { nick_name: item.name } }"
        ...
        <div>
          <img
            :src="item.image"
            alt=""
            style="width: 150px; height: 150px; max-height: 50%"
          />
        </div>
      </div>
    </div>
    ...
    getHeroes() {
      api
        ...
        .then((response) => {
          ...
          this.gotData = "yes";
          this.exampleItems = [...Array(this.nick_names.length).keys()].map(
            (i) => ({ id: i, name: this.nick_names[i], image: this.images[i] })
          );
        })
